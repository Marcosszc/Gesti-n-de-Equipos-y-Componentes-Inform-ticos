...
// Mapeo Supabase (tabla 'componentes') -> React state (fecha_registro -> fechaRegistro)
const mapFromDb = (row: any): ComponenteItem => ({
  id: String(row.id),
  codigo: row.codigo,
  nombre: row.nombre,
...
// Mapeo React state -> Supabase (tabla 'componentes') (fechaRegistro -> fecha_registro)
const mapToDb = (item: ComponenteItem | Omit<ComponenteItem, 'id'>) => ({
  codigo: item.codigo,
  nombre: item.nombre,
...
export default function App() {
  const [items, setItems] = useState<ComponenteItem[]>(INITIAL_ITEMS);
  ...
  // Consultar tabla 'componentes' en Supabase (SELECT)
    ...
    try {
      if (isSupabaseConfigured) {
        ...
        if (data && data.length > 0) {
          ...
          // Si la tabla remota está recién creada y vacía, se pueden inicializar los registros de prueba
          const initialPayload = INITIAL_ITEMS.map((it) => mapToDb(it));
          ...
          if (!insertError && inserted && inserted.length > 0) {
            setItems(inserted.map(mapFromDb));
          } else {
            setItems(INITIAL_ITEMS);
          }
        }
      } else {
        // Modo local si no se han configurado aún las variables de entorno
      ...
      console.error('Error al consultar tabla componentes en Supabase:', err);
      showToast('error', `Error en Supabase: ${err.message || 'Verifica la conexión'}`);
      const saved = localStorage.getItem(STORAGE_KEY);
  ...
  // Respaldo secundario en localStorage si Supabase no está configurado
  useEffect(() => {
    if (!isSupabaseConfigured) {
      try {
  ...
  // Handlers for CRUD (Supabase INSERT, UPDATE, DELETE)
  const handleSaveItem = async (itemData: Omit<ComponenteItem, 'id'> | ComponenteItem) => {
    try {
      if ('id' in itemData && itemData.id) {
        // C. Modificación / Actualizar (UPDATE en tabla 'componentes')
        if (isSupabaseConfigured) {
          const dbPayload = mapToDb(itemData);
          const { data, error } = await supabase
            .from('componentes')
            .update(dbPayload)
            .eq('id', itemData.id)
          ...
          const updatedItem = data && data[0] ? mapFromDb(data[0]) : (itemData as ComponenteItem);
          setItems((prev) =>
            prev.map((it) => (it.id === itemData.id ? updatedItem : it))
          );
          showToast('success', `Componente "${itemData.nombre}" (${itemData.codigo}) actualizado en Supabase.`);
        } else {
          setItems((prev) =>
        ...
        // A. Alta / Crear (INSERT en tabla 'componentes')
        if (isSupabaseConfigured) {
          const dbPayload = mapToDb(itemData);
          const { data, error } = await supabase
            .from('componentes')
          ...
          if (data && data[0]) {
            const createdItem = mapFromDb(data[0]);
            setItems((prev) => [createdItem, ...prev]);
            showToast('success', `Nuevo componente "${createdItem.nombre}" registrado en Supabase con éxito.`);
          }
        } else {
      ...
      showToast('error', `Error al guardar en Supabase: ${error.message || 'Verifica la conexión'}`);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      // D. Baja / Eliminar (DELETE en tabla 'componentes')
      if (isSupabaseConfigured) {
        ...
        showToast('success', `Registro "${deletedName}" (${deletedCode}) eliminado de Supabase.`);
      } else {
        setItems((prev) => prev.filter((it) => it.id !== targetId));
      ...
      showToast('error', `Error al eliminar en Supabase: ${error.message || 'Error de conexión'}`);
    } finally {
  ...
  const handleResetData = async () => {
    if (!window.confirm('¿Deseas restablecer el inventario a los componentes de prueba?')) return;

    if (isSupabaseConfigured) {
      try {
        setIsLoading(true);
        await supabase.from('componentes').delete().neq('codigo', '___DUMMY_NEVER_MATCH___');
        const seedPayload = INITIAL_ITEMS.map((it) => mapToDb(it));
        const { data, error } = await supabase.from('componentes').insert(seedPayload).select();
        if (error) throw error;
        if (data) setItems(data.map(mapFromDb));
        showToast('success', 'Inventario restablecido en Supabase con éxito.');
      } catch (err: any) {
        console.error('Error al restablecer en Supabase:', err);
        showToast('error', `Error al restablecer en Supabase: ${err.message}`);
      } finally {
        setIsLoading(false);
      }
    } else {
                            ...
                            <td className="px-4 py-3 text-right whitespace-nowrap">
                              <div className="flex items-center justify-end gap-1.5">
                                <button
                                  ...
                                  className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-md border border-indigo-200 transition-colors"
                                >
                                ...
                                <button
                                  ...
                                  className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold text-rose-700 bg-rose-50 hover:bg-rose-100 rounded-md border border-rose-200 transition-colors"
                                >
      ...
      <footer className="border-t border-slate-200 bg-white py-4 mt-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-500">
          ...
          <div className="flex items-center gap-3">
            <span>Sistema de Gestión de Equipos y Componentes</span>
          </div>
        </div>
      </footer>
